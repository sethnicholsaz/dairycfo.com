...
function Trend({ current, prev }: { current: number | null; prev?: number | null }) {
  if (!current || !prev) return <Minus size={14} className="text-ink-300" />
...
export async function MarketSnapshot() {
  ...
  return (
    <div className="bg-cream-200 border border-cream-400 rounded-lg overflow-hidden my-6">
      {/* Header */}
      <div className="bg-green-800 text-cream-200 px-5 py-3 flex items-center justify-between">
      ...
      {/* Announced prices */}
      <div className="p-5">
        <p className="text-xs text-ink-300 uppercase tracking-wider mb-3 font-medium">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {rows.map((row) => (
            <div key={row.label} className="bg-white rounded p-3 border border-cream-400">
              ...
              <p className="font-mono font-bold text-green-800 text-lg leading-none">
        ...
        <div className="px-5 pb-5 border-t border-cream-400 pt-4">
          ...
          <div className="flex gap-2 flex-wrap">
            {futures.slice(0, 6).map((f) => (
              ...
              >
                <p className="text-xs text-ink-300">{f.month}</p>
                <p className="font-mono font-semibold text-green-800 text-sm">${f.price.toFixed(2)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-cream-300 px-5 py-2 text-xs text-ink-300 border-t border-cream-400">
